...
const globalStyle = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Pretendard', sans-serif; background: #fff; color: #111; -webkit-font-smoothing: antialiased; }
...
function Polyphonic() {
    ...
    const searchResults = useMemo(() => {
        ...
        return all.filter(item => {
            ...
            const content = Object.values(item).filter(v => typeof v === 'string' && !v.startsWith('data:image')).join(" ").toLowerCase();
        ...
        <div style={{ minHeight: "100vh" }}>
            ...
            <main style={{ maxWidth: 720, margin: "0 auto", paddingBottom: 100 }}>
                            ...
                            <div className="grid-layout">
                                {(activeTab === "composer" ? composers : activeTab === "genre" ? genres : instruments).map(item => (
                                    <div key={item.id} className="grid-card" onClick={() => { setSelectedItem(item); setView("detail"); }}>
                                        <img src={item.photo || 'https://via.placeholder.com/300'} className="grid-img" />
                ...
                {view === "hashtagPage" && (
                    <div style={{ padding: "20px 25px" }}>
                        ...
                        <h1 style={{ fontSize: 28, fontWeight: 900, marginBottom: 30 }}><span style={{ color: ACCENT }}>{targetHashtag}</span> 검색 결과</h1>
            ...
            {showModal.music && <div className="modal-overlay"><div className="modal-content scroll-container">
                ...
                <div style={{ marginBottom: 20 }}>
                    <label className="section-label" style={{ display: 'block', marginBottom: 8 }}>(10) 구성 분석</label>
                    <button onClick={() => setShowCompositionLayer(true)} style={{ width: '100%', padding: 12, background: '#f8f8f8', border: `1px dashed ${BORDER}`, borderRadius: 8, color: ACCENT, fontWeight: 600, cursor: 'pointer' }}>구성 분석 입력</button>
                </div>
                ...
                <button onClick={() => { setMusics(forms.music.id ? musics.map(m => m.id === forms.music.id ? forms.music : m) : [{ ...forms.music, id: Date.now() }, ...musics]); setShowModal({ ...showModal, music: false }); }} style={{ width: '100%', padding: 18, background: ACCENT, color: '#fff', border: 'none', borderRadius: 12, fontWeight: 800, cursor: 'pointer' }}>저장</button>
            </div></div>}
        ...
        <div style={{ padding: "20px 25px" }}>
            ...
            <div>
                {type === 'library' && (<>
                    ...
                    <Section label="감상 링크">{data.link ? <a href={data.link} target="_blank" style={{ color: ACCENT }}>{data.link}</a> : "-"}</Section>
    ...
    return (
        <div style={{ marginBottom: 22 }}>
            ...
            <div style={{ maxHeight: 100, overflowY: 'auto', border: `1px solid ${BORDER}`, marginTop: 8, borderRadius: 10, padding: 5 }}>
                {list.map(i => (
                    <div key={i.id} onClick={() => onSelect(i.id)} style={{ padding: '8px 12px', fontSize: 12, background: selectedIds.includes(i.id) ? ACCENT_LIGHT : 'none', cursor: 'pointer', borderRadius: 8 }}>
